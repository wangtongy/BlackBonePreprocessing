function Interpolation_mr(Input_folder,out, subject_list)
    cd(Input_folder)
    ...
    parfor i = 1:length(subject_list)
        ...
        nameIn =sprintf('%s/%s_correction_MR.nii.gz',Input_folder,subject_list{i});
        nameout = sprintf('%s/%s_nonorm_r1_inter.nii.gz',out,subject_list{i});
